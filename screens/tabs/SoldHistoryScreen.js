// screens/tabs/SoldHistoryScreen.js
...
const CheckoutItem = ({ item, index, theme, onViewDetails, onContactBuyer }) => {
          ...
          <View style={styles.infoSection}>
            ...
            <View style={styles.sellerInfo}>
              ...
              <Text style={[styles.sellerName, { fontFamily: fontFamily.medium }]} numberOfLines={1}>
                {' '}Buyer: {item.buyer_email || 'Unknown'}
              </Text>
            </View>
...
export default function SoldHistoryScreen({ navigation }) {
  ...
  useEffect(() => {
    const fetchSellerName = async () => {
      ...
      if (!error && data?.name) {
        ...
        console.log('✅ Seller name fetched:', data.name);
      } else {
        console.warn('⚠️ Could not fetch seller name:', error);
      }
    };
  ...
  // Fetch sold history when seller name is ready
  useEffect(() => {
    if (sellerName) fetchSoldHistory();
  }, [sellerName]);

  const fetchSoldHistory = async () => {
    if (!sellerName) {
      console.log('⚠️ Seller name not ready yet');
      return;
    }
    ...
    try {
      ...
      console.log(`📦 Fetched ${data?.length || 0} sold items for seller: ${sellerName}`);
      ...
      console.error('Error fetching sold history:', err.message);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const calculateStats = () => {
    const totalSold = history.length;
    const totalRevenue = history.reduce(
      (sum, item) => sum + parseFloat(item.price) * parseInt(item.quantity),
      0
    );
    const uniqueBuyers = [...new Set(history.map((item) => item.buyer_email))].length;
  ...
  const handleContactBuyer = async (item) => {
    try {
      const buyerEmail = item.buyer_email;
      
      console.log('📧 Contacting buyer:', buyerEmail);
      ...
      if (error || !buyerData) {
        ...
        showToast('Unable to find buyer contact');
        return;
      }
      ...
      navigation.navigate('MessagingScreen', {
        receiverId: buyerData.email,
        receiverName: buyerData.name,
        productContext: {
          id: item.id,
      ...
      showToast('Opening chat with buyer...');
    } catch (error) {
  ...
  const renderEmptyState = () => (
    ...
    >
      <View style={styles.emptyIconContainer}>
        <Icon name="shopping-bag" size={64} color={theme.iconPlaceholder} />
      </View>
            ...
            {selectedOrder && (
              <View style={styles.receiptBody}>
                ...
                <View style={styles.receiptSection}>
                  ...
                  <Text style={[styles.receiptValue, { fontFamily: fontFamily.bold }]}>
                    {selectedOrder.buyer_email || 'Unknown'}
                  </Text>
                </View>
